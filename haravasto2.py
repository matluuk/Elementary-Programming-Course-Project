...
#lisätty haravastoon: ikkunan ikoneiden ja taustakuvan lataus
def lataa_kuvat(polku):
    ...
    kuvat["tausta"] = pyglet.resource.image("tausta.png")
    kuvat["ikoni"] = pyglet.resource.image("icon.png")
    kuvat["ikoni2"] = pyglet.resource.image("icon2.png")
...
#lisätty haravastoon: taustan luonti kuvasta, ikonit ikkunalle ja otsikko
def luo_ikkuna(leveys=800, korkeus=600, kuva="tausta", otsikko='nimi'):
    """
    ...
    if grafiikka["ikkuna"] is None:
        grafiikka["ikkuna"] = pyglet.window.Window(leveys, korkeus, resizable=True, caption=otsikko)
        grafiikka["ikkuna"].set_icon(grafiikka["kuvat"]["ikoni"], grafiikka["kuvat"]["ikoni2"])
        grafiikka["tausta"] = pyglet.sprite.Sprite(
            grafiikka["kuvat"][kuva], batch=grafiikka["puskuri"], group=pyglet.graphics.OrderedGroup(0)
        )
...
#lisätty haravstoon: tasaus_x ja tasaus_y argumentit
def piirra_tekstia(teksti, x, y, vari=(0, 0, 0, 255), fontti="serif", koko=32, tasaus_x="left", tasaus_y="bottom"):
    """
    ...
    tekstilaatikko = pyglet.text.Label(teksti,
        ...
        color=vari,
        x=x, y=y,
        anchor_x=tasaus_x, anchor_y=tasaus_y
    )
    tekstilaatikko.draw()
...
#lisätty haravastoon
def lisaa_piirrettava_nelikulmio(x, y, w, h, vari=(100, 100, 100)):
    """
    Lisää piirrettävän nelikulmion auki olevaan piirtopuskuriin.
    """

    rectangle = pyglet.shapes.Rectangle(x, y, w, h, color=vari, batch=grafiikka["puskuri"])

    #käytettäisiin laatikoiden pyörittämiseen
    #rectangle.anchor_x = rectangle.width / 2
    #rectangle.anchor_y = rectangle.height / 2
    #rectangle.rotation = kulma

    grafiikka["spritet"].append(rectangle)

#lisätty haravastoon
def lisaa_piirrettava_ympyra(x, y, r, vari=(100, 100, 100), lapinakyvyys=255):
    """
    Lisää piirrettävän ympyrän auki olevaan piirtopuskuriin.
    """
    
    circle = pyglet.shapes.Circle(x, y, r, color=vari, batch=grafiikka["puskuri"])
    circle.opacity = lapinakyvyys
    grafiikka["spritet"].append(circle)
